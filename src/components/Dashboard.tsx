...
export function Dashboard() {
  ...
  return (
    <div className="dashboard-container min-h-screen bg-[#f6f4f1] p-8">
      {/* Dashboard Header */}
      ...
      <div className="dashboard-stats grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={index} className="dashboard-stat-card bg-white rounded-xl p-6 border border-[#b29e84]/20">
              <div className={`dashboard-stat-icon w-12 h-12 rounded-lg ${stat.color} flex items-center justify-center mb-4`}>
                <Icon className="w-6 h-6" />
              </div>
              <p className="dashboard-stat-value text-2xl font-bold text-[#253439] mb-1">{stat.value}</p>
      ...
      <div className="dashboard-courses mb-8">
        <div className="dashboard-courses-header flex items-center justify-between mb-4">
          ...
          <button className="dashboard-courses-view-all text-[#fbb80f] hover:text-[#253439] text-sm font-medium flex items-center gap-1">
            Ver todos
        ...
        <div className="dashboard-courses-grid grid grid-cols-1 lg:grid-cols-3 gap-6">
          {enrolledCourses.map((course) => (
            <div key={course.id} className="dashboard-course-card bg-white rounded-xl border border-[#b29e84]/20 overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="dashboard-course-content p-6">
                <div className="dashboard-course-badge flex items-center gap-2 mb-2">
                  <span className="dashboard-course-level text-xs font-semibold text-white bg-[#253439] px-2 py-1 rounded">
                    {course.level}
                  </span>
                </div>
                <h3 className="dashboard-course-title font-semibold text-[#253439] mb-2">{course.title}</h3>
                <p className="dashboard-course-progress-text text-sm text-[#7c898b] mb-4">
                ...
                <div className="dashboard-course-progress mb-4">
                  ...
                  <div className="dashboard-course-progress-bar-container w-full bg-[#b29e84]/20 rounded-full h-2">
                    <div 
                ...
                <div className="dashboard-course-next-lesson bg-[#f6f4f1] rounded-lg p-3 mb-4">
                  <p className="dashboard-course-next-lesson-label text-xs text-[#7c898b] mb-1">Próxima lição:</p>
                  <p className="dashboard-course-next-lesson-title text-sm font-medium text-[#253439]">{course.nextLesson}</p>
                </div>

                <button className="dashboard-course-continue-button w-full bg-[#fbb80f] text-white py-2.5 rounded-lg hover:bg-[#253439] transition-colors flex items-center justify-center gap-2">
      ...
      <div className="dashboard-content-grid grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="dashboard-recent-lessons bg-white rounded-xl border border-[#b29e84]/20 p-6">
          <h2 className="dashboard-section-title text-xl font-semibold text-[#253439] mb-4">Lições Recentes</h2>
          <div className="dashboard-recent-lessons-list space-y-3">
            {recentLessons.map((lesson) => (
              <div key={lesson.id} className="dashboard-lesson-item flex items-center gap-4 p-4 bg-[#f6f4f1] rounded-lg hover:bg-[#b29e84]/10 transition-colors cursor-pointer">
                ...
                <div className="dashboard-lesson-info flex-1">
                  <p className="dashboard-lesson-title font-medium text-[#253439]">{lesson.title}</p>
        ...
        <div className="dashboard-upcoming-classes bg-white rounded-xl border border-[#b29e84]/20 p-6">
          <h2 className="dashboard-section-title text-xl font-semibold text-[#253439] mb-4">Próximas Aulas</h2>
          <div className="dashboard-upcoming-classes-list space-y-3">
            {upcomingClasses.map((cls) => (
              <div key={cls.id} className="dashboard-class-item flex items-start gap-4 p-4 bg-[#f6f4f1] rounded-lg hover:bg-[#b29e84]/10 transition-colors cursor-pointer">
                <div className="dashboard-class-icon w-10 h-10 bg-[#fbb80f]/20 rounded-lg flex items-center justify-center">
                  <Calendar className="w-5 h-5 text-[#fbb80f]" />
                </div>
                <div className="dashboard-class-info flex-1">
                  <p className="dashboard-class-title font-medium text-[#253439]">{cls.title}</p>
                  <p className="dashboard-class-teacher text-sm text-[#7c898b]">{cls.teacher}</p>
                  <p className="dashboard-class-date text-sm text-[#7c898b] mt-1">
                    {new Date(cls.date).toLocaleDateString('pt-BR')} às {cls.time}
                  </p>
                </div>
                <button className="dashboard-class-join-button text-[#fbb80f] hover:text-[#253439] text-sm font-medium">
                  Entrar
                </button>
              </div>
            ))}
      ...
      {/* Teacher Section */}
      <div className="dashboard-teacher-section mt-6 bg-gradient-to-br from-[#253439] to-[#7c898b] rounded-xl p-8 text-white">
